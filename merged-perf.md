...
- [ ] **Reconcile harness, baseline, and gate.** `benchmark/bench.dart` emits
...
- [ ] **Add a scale matrix.** Workloads at 1k / 10k / 100k / 1M rows; narrow,
...
- [ ] **Report distributions, not means.** p50/p95/p99, min/max, std-dev,
...
- [ ] **Record artifact + environment metadata with every run:** source
...
- [ ] **Add physical-work counters at the worker boundary:** primary rows
...
- [ ] **Add an indexed range/prefix workload + an indexed-equality@100k
...
- [ ] **Split timings by layer.** Extend `benchmark/boundary.dart` beyond the
...
- [ ] Add a grouped previous-row read by table/key, or a Rust batch prepare/
...
- [ ] Preserve repeated-key semantics: each mutation's "previous" must be the
  state immediately before that mutation, not before the whole batch.
- [ ] Preserve one writer, one encoded batch, one `redb` transaction, LSN
  ordering, change-log rows, sync-state rows, index maintenance, and reactive
  deltas.
- [ ] Add a regression test that fails if worker request count scales with
...
- [ ] Remove Dart-side per-write LSN reads and snapshot create/dispose around
...
- [ ] Count snapshots and metadata writes per `put`/`delete`/`bulkWrite`/
...
- [ ] Preserve `Uint8List` end-to-end through dispatch and worker messages
  where FRB ownership/lifetime allow; remove redundant `List<int>.from` and
  `.toList()` calls.
- [ ] Prototype a **packed result buffer**: one contiguous `Uint8List` plus
...
- [ ] Add capability-aware direct routes for ordinary point reads, `getMany`,
...
- [ ] `_CollectionImpl.getMany` (`database_impl.dart`) currently opens and
  disposes a snapshot even though a batched native `getMany` exists; route it
  through the non-snapshot op.
- [ ] Verify direct routes do not weaken the consistency promised by the
...
- [ ] Compare field names as borrowed UTF-8 byte slices against
  `field.as_bytes()` without allocating a `String`; only decode on a match.
- [ ] Add `find_fields(buf, &[(&str, &mut Option<RowValue>)])` that walks the
...
- [ ] Add `find_field_bytes` returning the raw encoded value bytes so
...
- [ ] Compile predicate field names and operation structure once per
  query/registration.
- [ ] Keep `skip_value`'s allocation-free traversal for unreferenced nested
...
- [ ] **Use `insert`/`remove` return values.** redb 4.1.0's
...
- [ ] **Cache table handles per batch.** Keep a per-transaction
  `HashMap<String, Table>` so a large batch does not re-`open_table` (string
  compare + handle construction) per op.
- [ ] **Batch durable-index maintenance.** Open `__gecko_index` once per
...
- [ ] **Fix `table_definition` allocation.** It does
...
- [ ] **`DeleteRange`/`Clear` amplification.** `apply_batch_impl` materializes
...
- [ ] **Make change-log pruning incremental.** `prune_change_log` iterates the
...
- [ ] **Design a separate, versioned order-preserving index-key encoding** for
  ...
  and repair behavior.
- [ ] **Implement exact lower/upper bounds and reverse scans**; stop early for
  `limit`/`offset` when ordering proves correctness.
- [ ] **Skip the predicate recheck when the index covers all filters**
  (`predicate.covers(index_fields)`); today every indexed candidate row still
  gets a row fetch + full predicate test.
- [ ] **Remove the ordered-query fallback scan** with index presence/missing
...
- [ ] **Add a reverse index iterator** for descending sorts (m4 explicitly
  deferred this; today DESC without an eq bound uses top-K over a full scan).
- [ ] **Composite indexes** (option): extend `durable_index_key` to
  ...
  intersection.
- [ ] **Improve candidate intersection** for multi-index: measure set sizes,
...
- [ ] **Push page size/offset into native limits** wherever semantics allow
...
- [ ] **Add a real planner cost decision**: count index entries visited vs
...
- [ ] Add cross-language golden fixtures and crash/reopen/repair tests for
...
- [ ] **Send only deltas when the consumer needs them.** Extend the
  ...
  regression test.
- [ ] **Pass post-commit row bytes to the registry.** `apply_batch_impl`
...
- [ ] **Deduplicate affected `(table, key)` pairs** before registry
  evaluation (repeated keys arrive in one batch), while preserving
  final-state and diff semantics.
- [ ] **Optimize sorted registries.** Evaluate a `BTreeMap<(encoded sort
...
- [ ] **Share changed-row reads among registrations** (one transaction-local
  post-mutation value per key per batch).
- [ ] **Implement explicit backpressure.** `ChangeBus.maxBuffered` (1024) and
...
- [ ] **Benchmark large watched results** (1/1k/10k/100k rows; one-row update,
...
- [ ] **Measure windowed and relationship watches.** `limit`/`offset` queries
...
- [ ] **Fix negative lookup caching.** `RawEngine` stores `List<int>?` in the
...
- [ ] **Invalidate only affected cache entries.** `commitBatch` clears the
...
- [ ] **Reduce byte-key churn.** `ByteKey.bytes` returns a fresh `Uint8List`
...
- [ ] **Verify model mapping stays a measured lower bound.** Time wire decode,
  map copy, and `fromRow` separately; do not promise end-to-end gains on
  workloads where model construction dominates.
- [ ] **Measure worker contention.** `native_worker_client.dart` processes
...
- [ ] **Audit MVCC snapshot hygiene.** The worker keeps every `ReadTransaction`
...
- [ ] **Sync transitions and `changesSince`.** `database_impl.dart` still
...
- [ ] **Reduce change-record write amplification.** Reuse encoded change-record
  bytes across change log and sync state; measure primary/metadata/index/total
  bytes per transaction.
- [ ] **Make migrations bounded and resumable.** `_SchemaApiImpl.
...
- [ ] **Attachment metadata.** `orphaned()` scans all metadata in Dart with
...
- [ ] **Encryption.** `rust/src/crypto_storage.rs` AES-256-GCM-encrypts each
...
- [ ] **Web/OPFS.** OPFS uses synchronous `FileSystemSyncAccessHandle` I/O
...
- [ ] **Release profile.** `rust/Cargo.toml` sets `lto = true` only. Add
...
- [ ] **Artifact freshness.** benchmark helpers resolve
...
- [ ] **A/B compiler settings in isolated runs** and keep generated FRB +
