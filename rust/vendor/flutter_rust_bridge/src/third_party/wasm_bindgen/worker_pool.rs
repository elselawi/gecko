...
impl Default for WorkerPool {
    fn default() -> Self {
        // gecko_db vendored patch: tolerate environments where Web Workers
        // cannot be created (headless browsers, strict CSP, sandboxed iframes)
        // or where the non-shared wasm memory cannot be transferred to a
        // worker (DataCloneError). The pool starts empty; any attempt to use
        // it later fails gracefully instead of panicking at startup.
        //
        // gecko_db never dispatches CPU-bound (`spawn_blocking`) work on the
        // web — every FRB call is async and runs on the main thread through
        // the async runtime — so an empty pool is never exercised.
        match Self::new(None, None, None, None) {
            Ok(pool) => pool,
            Err(_) => {
                Self::new(Some(0), Some(String::new()), None, None)
                    .expect("fail to create empty WorkerPool")
            }
        }
    }
}
