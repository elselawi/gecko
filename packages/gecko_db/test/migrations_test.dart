...
void main() {
  group('schema version stamping', () {
    test('new database starts unversioned; stamp is idempotent', () async {
      final db = await openNativeTestDatabase('migrate-a');
  ...
  group('multi-step migration', () {
    test(
      'consecutive steps run in order to the final version and shape',
      () async {
        final db = await openNativeTestDatabase('migrate-c');
    ...
    test(
      ...
      () async {
        final db = await openNativeTestDatabase('migrate-d');
        final col = db.collection<_Rec>(
          'items',
          toRow: _toRow,
          fromRow: _fromRow,
    ...
    test('wrong from-version is a typed migration error', () async {
      final db = await openNativeTestDatabase('migrate-e');
      await db.schema.stamp(1);
  ...
  group('chained migration and stable IDs', () {
    test('v1 to v3 chain keeps record IDs stable across a rewrite', () async {
      final db = await openNativeTestDatabase('migrate-i');
      final col = db.collection<_Rec>(
        'items',
        toRow: _toRow,
        fromRow: _fromRow,
  ...
  group('additive fast path (no full rewrite)', () {
    test(
      'additive migration bumps version without rewriting any record',
      () async {
        final db = await openNativeTestDatabase('migrate-f');
        final col = db.collection<_Rec>(
          'items',
          toRow: _toRow,
          fromRow: _fromRow,
    ...
    test(
      ...
      () async {
        final db = await openNativeTestDatabase('migrate-g');
        final col = db.collection<_Rec>(
          'items',
          toRow: _toRow,
          fromRow: _fromRow,
  ...
  group('open-time compatibility gate', () {
    test(
      ...
      () async {
        final root = await Directory.systemTemp.createTemp('gecko-migrations-');
