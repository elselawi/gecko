...
Future<void> _seed(DatabaseImpl db, String table, {List<String>? indexFields}) async {
...
void main() {
  ...
  void runSuite(String label, Future<DatabaseImpl> Function(String tag) open) {
    group('M3 read path ($label)', () {
      test('iterate() matches findAll() and routes through the fast path', () async {
        ...
        final indexed = _coll(db, 't', indexFields: ['nick']).where({'nick': 'g3'});
      ...
      test('count() matches findAll().length and pushes the aggregate on native', () async {
      ...
      test('distinct() matches the Dart set semantics on both backends', () async {
      ...
      test('first() and findPage() stay correct on the fast path', () async {
        ...
        // Pages are disjoint.
        expect(page.map((r) => r.id).toSet().intersection(page2.map((r) => r.id).toSet()), isEmpty);
        await db.close();
      });

      test('getMany(ids) returns rows in input order, skipping absent ids', () async {
        ...
        expect((await col.getMany(['r5'])).single.id, (await col.get('r5'))!.id);
        await db.close();
      });

      test('getMany(ids) observes the transaction overlay inside a txn', () async {
        final db = await open('getmany-txn');
        await _seed(db, 't');
        final col = _coll(db, 't');
        await db.writeTxn((txn) async {
          final tcol = txn.collection<_Rec>('t', toRow: _toRow, fromRow: _fromRow, id: _id);
      ...
      test('relationship children still resolve via the batched getMany path', () async {
  ...
  runSuite(
    'in-memory',
    (tag) async => DatabaseImpl.open('mem://m3-$tag', useInMemory: true),
  );
