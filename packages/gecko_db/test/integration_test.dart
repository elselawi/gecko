...
void main() {
  group('integration: schema + Tier 1 CRUD', () {
    late DatabaseImpl db;

    setUp(() async {
      db = await openNativeTestDatabase('integration');
    });
  ...
  group('integration: patch preserves missing/null', () {
    late DatabaseImpl db;

    setUp(() async {
      db = await openNativeTestDatabase('integration-b');
    });
  ...
  group('auto-assigned ids', () {
    test('are unique across a large batch of inserts', () async {
      final db = await openNativeTestDatabase('integration-c');
      final col = db.collection<_Rec>(
        't',
        toRow: _toRow,
        fromRow: _fromRow,
  ...
  group('unknown stored fields survive round-trip', () {
    test('fields absent from fromRow mapping are preserved on write', () async {
      final db = await openNativeTestDatabase('integration-d');
      final col = db.collection<_Rec>(
        't',
        toRow: _toRow,
        fromRow: _fromRow,
