// Workstream 8 — sustained soak (reliability under continuous mixed load).
//
...
void main() {
  ...
  test('sustained mixed workload with encryption, compaction, reopen cycles',
      () async {
    ...
    try {
      ...
      final model = <String, Map<int, _Record>>{
        'events': {},
        'notes': {},
      };
      ...
      try {
        for (var cycle = 0; cycle < _cycles; cycle++) {
          ...
          final modelTag7 =
              model['events']!.values.where((r) => r.tag == 7).length;
          expect(tag7.length, modelTag7,
              reason: 'cycle $cycle: indexed tag==7 count drift');
          for (final r in tag7) {
            expect(r.tag, 7);
          }
          final all = await events.getAll();
          expect(all.length, model['events']!.length,
          ...
          expect(watchEvents, greaterThanOrEqualTo(1),
          ...
          if (cycle % 3 == 0) {
            ...
            await db.schema.migrateStep(
              MigrationStep(name: 'soak-$v', fromVersion: v, toVersion: v + 1),
            );
          ...
          if (cycle % 2 == 1) {
            ...
            expect((await events2.getAll()).length, model['events']!.length,
        ...
        for (final entry in model['events']!.entries) {
          expect(byId[entry.key], isNotNull,
              reason: 'final sweep missing id ${entry.key}');
          expect(byId[entry.key]!.payload, entry.value.payload,
        ...
        expect(sample.contains('payload-'), isFalse,
            reason: 'encrypted file leaked plaintext payload');
        expect(sample.contains('note-'), isFalse,
            reason: 'encrypted file leaked plaintext note');
        expect(sample.contains('patched-'), isFalse,
