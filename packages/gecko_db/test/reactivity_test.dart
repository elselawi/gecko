// incremental reactivity qualification 
//
...
void main() {
  group('incremental watchAll', () {
    test(
      'updates the cache incrementally without re-scanning the collection',
      () async {
        final db = await openNativeTestDatabase('reactivity-all');
        await _seed(db, 400);
    ...
    test('a whole-table clear resets the emission to empty', () async {
      final db = await openNativeTestDatabase('reactivity-clear');
      await _seed(db, 30);
      final lengths = <int>[];
      final sub = _coll(
        db,
  ...
  group('incremental watchAllDiff', () {
    test(
      'reports added, updated, and removed from incremental updates',
      () async {
        final db = await openNativeTestDatabase('reactivity-diff');
        await _seed(db, 20);
  ...
  group('incremental watch lifecycle', () {
    test('cancelling the subscription stops all further updates', () async {
      final db = await openNativeTestDatabase('reactivity-cancel');
      await _seed(db, 10);
      final emissions = <List<String>>[];
      final sub = _coll(db).watchAll().listen(
        (list) => emissions.add([for (final r in list) r.id]),
  ...
  group('incremental watchAllDiff', () {
    test('an idempotent put with an unchanged value emits no diff', () async {
      final db = await openNativeTestDatabase('reactivity-idempotent');
      await _seed(db, 5);
      final diffs = <CollectionDiff<_Rec>>[];
      final sub = _coll(db).watchAllDiff().listen(diffs.add);
      await _waitFor(() => diffs.isNotEmpty);
  ...
  group('incremental filtered query watch', () {
    test(
      'updates matching rows incrementally without re-scanning the collection',
      () async {
        final db = await openNativeTestDatabase('reactivity-query');
        await _seed(db, 600);
        final emissions = <List<String>>[];
    ...
    test('emits once per batch (coalesced), not once per key', () async {
      final db = await openNativeTestDatabase('reactivity-coalesce');
      await _seed(db, 10);
      final emissions = <int>[];
      final sub = _coll(db)
          .where({'group': 'g0'})
  ...
  group('incremental sorted query watch', () {
    test('keeps emissions in comparator order after updates', () async {
      final db = await openNativeTestDatabase('reactivity-sorted');
      final col = _coll(db);
      for (var i = 0; i < 40; i++) {
