// read-path completion + getMany 
//
// The suite runs against the native file backend and locks the contracts:
...
void main() {
  ...
  runSuite('native file', (tag) async {
    final dir = await Directory.systemTemp.createTemp('gecko-readpath-$tag-');
    addTearDown(() async {
      try {
  ...
  group('native snapshot error paths', () {
    test('getMany/count/distinct on a dropped snapshot fail typed', () async {
      final dir = await Directory.systemTemp.createTemp('gecko-readpath-err-');
      addTearDown(() async {
        try {
          await dir.delete(recursive: true);
        } catch (_) {}
