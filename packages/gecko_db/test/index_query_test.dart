// durable indexes, range-index support, and the snapshot-bound
...
void main() {
  ...
  void runSuite(String label, Future<DatabaseImpl> Function(String tag) open) {
    group('range index + cursor ($label)', () {
      test(
      ...
      test('indexed range + prefix + equality intersect together', () async {
      ...
      test('unindexed range query falls back to a full scan', () async {
        ...
        // (nativeFilteredScan — step 2). Neither uses a secondary index.
        expect(
          q.lastPlan,
  ...
  runSuite('native file', (tag) async {
    final dir = await Directory.systemTemp.createTemp('gecko-index-$tag-');
    addTearDown(() async {
      try {
  ...
  group('durable indexes (native file)', () {
    late Directory dir;
    late String path;

    setUp(() async {
      dir = await Directory.systemTemp.createTemp('gecko-index-native-');
  ...
  // native query fast path: indexed equality traverses the durable
  // `__gecko_index` table in one FRB hop instead of N per-id point reads.
  group('native query fast path (native file)', () {
    late Directory dir;
    late String path;

    setUp(() async {
      dir = await Directory.systemTemp.createTemp('gecko-query-');
      path = '${dir.path}${Platform.pathSeparator}db.redb';
    });
    ...
    test(
      ...
      () async {
        ...
        // Single covered equality filter — the native path.
    ...
    test('indexed candidate routes avoid a native full scan', () async {
    ...
    test(
      ...
      () async {
        ...
        final result = await q.findAll();
        // every sorted query routes through Rust. The sort field (age) is
    ...
    test('fast path uses one backend hop, not N point reads', () async {
      ...
      // (the baseline showed the N+1 path at ~33 ms for 1000 ids;
