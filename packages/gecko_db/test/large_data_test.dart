// large-data qualification (reliability / performance sanity).
//
...
class _Row {
  ...
  static _Row fromMap(Object? row) {
    ...
    return _Row(map['id'] as int, map['group'] as int, map['value'] as String);
  }
}
...
void main() {
  ...
  test(
    '100k+ rows with secondary index on native redb',
    () async {
      ...
      try {
        ...
        expect(
          pages,
          greaterThan(100),
          reason: 'pagination must span many pages',
        );
    ...
    try {
      ...
      for (var start = 0; start < n; start += 500) {
        ...
        for (var i = start; i < end; i++) {
          ...
          expected['m$i'] = row;
          mutations.add(
            BulkMutation.put(table: 'multi', key: 'm$i', value: row),
          );
        }
      ...
      try {
        ...
        final hits = await coll2
            .where()
            .filter('a', expected['m0']!['a'])
            .findAll();
  ...
  test(
    'large pending-sync change log survives close/reopen',
    () async {
      ...
      try {
        ...
        expect(
          pending.length,
          n,
          reason: 'all local writes must be pending sync',
        );
        ...
        try {
          ...
          expect(
            pending2.length,
            n,
            reason: 'pending sync must survive reopen',
          );
  ...
  test('hundreds of attachments with blob de-duplication', () async {
    final dir = await Directory.systemTemp.createTemp('gecko-qual-att-');
    final db = await Database.open(
  ...
  test(
    'long ordered migration chain over a large dataset',
    () async {
      ...
      try {
        ...
        final additive = [
          for (var v = 1; v <= 12; v++)
            MigrationStep(name: 'step-$v', fromVersion: v - 1, toVersion: v),
        ];
        final (applied, target) = await db.schema.migrate(
          MigrationPlan(steps: additive, targetVersion: 12),
        );
        expect(applied, 12);
        expect(target, 12);
        ...
        expect(
          rewritten,
          n,
          reason: 'every record must pass through the rewriter',
        );
