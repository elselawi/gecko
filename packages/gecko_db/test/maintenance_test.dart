// compaction, maintenance state machine, and diagnostics tests.
//
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('gecko-maintenance-');
  ...
  group('maintenance state machine', () {
    test(
  ...
  group('diagnostics', () {
    ...
    test(
      'per-stage timers attribute full-scan vs index-served costs',
      () async {
        final db = await openNative(
          config: DatabaseConfig(
            nativeLibraryPath: nativePath,
        ...
        // predicate is pushed to Rust (step 2), so only matching rows
    ...
    test(
      'per-stage timers report the sort stage on ordered queries',
      () async {
        final db = await openNative(
          config: DatabaseConfig(
            nativeLibraryPath: nativePath,
        ...
        // the sort executes in Rust (top-K), so the Dart `sort` stage is
        // zero and the sort cost is inside `backendRead`.
        expect(t.sort, 0, reason: 'no Dart-side sort executes');
